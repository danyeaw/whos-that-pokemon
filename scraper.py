    ...
    async with httpx.AsyncClient(headers=headers, timeout=15.0) as client:
        ...
        for i in range(1, MAX_CARD_NUMBER + 1):
            ...
            task = asyncio.create_task(
                download_card(client, url, filename, i)
            )
            tasks.append(task)
...
async def download_card(client: httpx.AsyncClient, url: str, filepath: Path, card_num: int):
    try:
        ...
        if e.response.status_code != 404:  # Ignore 404s as we expect some numbers to not exist
